...
public abstract class NoiseChunkGeneratorMixin {
  ...
  @Redirect(
      method =
          "populateNoise(Lnet/minecraft/world/gen/chunk/Blender;Lnet/minecraft/world/gen/StructureAccessor;Lnet/minecraft/world/gen/noise/NoiseConfig;Lnet/minecraft/world/chunk/Chunk;II)Lnet/minecraft/world/chunk/Chunk;",
      at =
          @At(
              value = "INVOKE",
              target =
  ...
  private BlockState noisium$populateNoiseWrapSetBlockStateOperation(
      @NotNull ChunkSection chunkSection,
      int chunkSectionBlockPosX,
      int chunkSectionBlockPosY,
      int chunkSectionBlockPosZ,
      @NotNull BlockState blockState,
      boolean lock) {
    // Update the non empty block count to avoid issues with MC's lighting engine and other systems
    // not recognising the direct palette storage set
  ...
  /**
   * @author Steveplays28
   * @reason Improved chunk locking and unlocking speed by getting the chunk section array directly
   *     from the chunk and by replacing {@code foreach} with {@code fori}.
   */
  @Overwrite
  private @Nullable Chunk method_38332(
      @NotNull Chunk chunk,
    ...
    final int startingChunkSectionIndex =
        chunk.getSectionIndex(
            generationShapeHeightFloorDiv * generationShapeConfig.verticalCellBlockCount()
                - 1
                + minimumY);
    ...
    try {
      ...
      // Replace an enhanced for loop with a fori loop and reuse the chunk sections array used when
      // locking the chunk sections
      for (int chunkSectionIndex = startingChunkSectionIndex;
          chunkSectionIndex >= minimumYChunkSectionIndex;
          --chunkSectionIndex) {
